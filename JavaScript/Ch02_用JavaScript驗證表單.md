...
\t           匹配一個製表元=\x09、\cI
\v           匹配一個垂直製表元=\x0b、\cK
\w           匹配包括下劃線的任何單字字元='[A-Za-z0-9_]'
\W           匹配任何非單字字元='[^A-Za-z0-9_]'
\xn          匹配n，其中n為十六進制，值必須為確定的兩個數字長
\num         匹配num，其中num是一個正整數
\n           標示一個八進制值或一個向後引用
\nm          標示一個八進制值或一個向後引用
\nml         如果n為八進制數字(0~3)，且m和l均為八進制數字，則匹配八進制值為nml
\un          匹配n，其中n是用一個用4個十六進制數字表示的Unicode字元
```
### 2.3.3 正則驗證輸入信箱
## 2.4 改善用戶體驗
### 2.4.1 甚麼是用戶體驗
